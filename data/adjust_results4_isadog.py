...
def adjust_results4_isadog(results_dic, dogfile):
  ...
  """           
# Creates dognames dictionary for quick lookup
  dognames_dic = dict()
  ...
  with open(dogfile, "r") as f:
    for line in f:
      ...
      # Adds dog_name to dognames_dic if not already present
      if dog_name not in dognames_dic:
          dognames_dic[dog_name] = 1

    # Process through results_dic to check if labels are dogs
    for key in results_dic:
        # Pet Image Label is a Dog (index 0)
        is_pet_dog = 1 if results_dic[key][0] in dognames_dic else 0
        
        # Classifier Label is a Dog (index 1)
        is_classifier_dog = 1 if results_dic[key][1] in dognames_dic else 0
        
        # Append dog indicators to results_dic value list (indices 3 and 4)
